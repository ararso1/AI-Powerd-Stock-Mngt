...
export class InventoryService {
  ...
  private buildInventoryFilterQb(query: InventoryListQueryDto) {
    ...
    if (query.lotId) {
      qb.andWhere('stock.lot_id = :lotId', { lotId: query.lotId });
    }
    if (query.form || query.cropYear || query.grade) {
      qb.innerJoin('stock.lot', 'lot_filter');
      if (query.form) {
        qb.andWhere('lot_filter.form = :form', { form: query.form });
      }
      if (query.cropYear) {
        qb.andWhere('lot_filter.crop_year = :cropYear', {
          cropYear: query.cropYear,
        });
      }
      if (query.grade) {
    ...
    if (query.search?.trim()) {
      ...
      qb.andWhere(
        `(EXISTS (
            SELECT 1 FROM items item_filter
            WHERE item_filter.id = stock.item_id
              AND (item_filter.description ILIKE :term OR item_filter.sku ILIKE :term)
          )
          OR EXISTS (
            SELECT 1 FROM lots lot_search
            WHERE lot_search.id = stock.lot_id
              AND lot_search.code ILIKE :term
          ))`,
        { term },
      );
    }
  ...
  async createAdjustment(dto: CreateStockAdjustmentDto, userId?: string) {
    ...
    const coffeeOutReasons = new Set<StockAdjustmentReason>([
      StockAdjustmentReason.MOISTURE_LOSS,
      StockAdjustmentReason.SHRINKAGE,
      StockAdjustmentReason.QC_REJECT,
    ]);

    if (coffeeOutReasons.has(dto.reason) && !dto.lotId) {
      throw new BadRequestException(
        `${dto.reason} requires a lot (lotId)`,
      );
    }
    if (isCoffee && !dto.lotId) {
      throw new BadRequestException(
        'Coffee stock adjustments require a lot (lotId)',
      );
    }
    if (
      coffeeOutReasons.has(dto.reason) &&
    ...
    const lotId = dto.lotId ?? null;
    if (lotId) {
      const lot = await this.lotRepo.findOne({ where: { id: lotId } });
      if (!lot) throw new BadRequestException('Lot not found');
      if (lot.itemId && lot.itemId !== dto.itemId) {
        throw new BadRequestException('Lot does not match item');
      }
      if (lot.locationId && lot.locationId !== dto.locationId) {
        throw new BadRequestException(
          'Lot is not at the selected location',
        );
      }
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      if (lotId) {
        ...
        if (lot) {
          const nextQty = Math.max(0, parseFloat(lot.quantity) + delta);
          ...
          await eventRepo.save(
            eventRepo.create({
              ...
              notes:
                dto.notes?.trim() ||
                `Stock ${dto.direction} · ${dto.reason}`,
    ...
    return this.adjustmentRepo.findOne({
      where: { id: result.id },
      relations: { item: true, location: true, lot: true, createdBy: true },
    });
  }
