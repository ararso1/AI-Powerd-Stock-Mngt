...
export class Sale extends UuidBaseEntity {
  ...
  @Column({
    type: 'enum',
    enum: SaleChannel,
    enumName: 'sales_channel_enum',
    default: SaleChannel.LOCAL,
  })
  channel: SaleChannel;

  @Column({ name: 'currency_code', type: 'varchar', length: 3, default: 'ETB' })
  currencyCode: string;

  /** Local currency per 1 unit of sale currency (e.g. ETB per USD). */
  @Column({
    name: 'fx_rate',
    type: 'decimal',
    precision: 14,
  ...
  @ManyToOne(() => ExportContract, { nullable: true })
