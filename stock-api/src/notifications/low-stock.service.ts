...
export class LowStockService {
  ...
  async evaluate(
    ...
    const lotLabel = stock.lot?.code ? ` [${stock.lot.code}]` : '';

    if (current <= 0 && previousQuantity > 0) {
      await this.sendAlert(stock, {
        title: 'Out of stock',
        message: `${stock.item.description}${lotLabel} at ${stock.location.name}: out of stock (0 remaining)`,
    ...
    if (
      ...
      await this.sendAlert(stock, {
        title: 'Low stock',
        message: `${stock.item.description}${lotLabel} at ${stock.location.name}: ${current} remaining (reorder point: ${reorderPoint})`,
