...
export class ReportsService {
  ...
  async inventoryAging() {
    ...
    const items = stockLevels.map((stock) => {
      ...
      totalValue += value;
      const ageAnchor = stock.lot?.createdAt ?? stock.updatedAt;
      return {
        itemId: stock.itemId,
        itemDescription: stock.item.description,
        sku: stock.item.sku ?? undefined,
        ...
        lotId: stock.lotId ?? undefined,
        lotCode: stock.lot?.code,
        form: stock.lot?.form,
        grade: stock.lot?.grade ?? undefined,
        cropYear: stock.lot?.cropYear ?? undefined,
        ageDays: Math.floor(
          (today.getTime() - ageAnchor.getTime()) / (1000 * 60 * 60 * 24),
        ),
      };
    });
